...
def commandline_args():
    ...
    parser.add_argument('-output_sample_umap_coords', type = str, nargs = 1, help = 'The path to export a sample\'s log transformed expression matrix.')
...
def generate_sample_umap_coords_output_file(sample_cluster_dict, output_dir):
    """
    ...
    for sample_key, adata in sample_cluster_dict.items():
        umap_coord = pd.DataFrame(adata.obsm.X_umap, index=adata.obs.index.tolist())
        umap_coord.columns = ['x', 'y']
        umap_coord.to_csv(output_dir + sample_key + "_umap_coords.tsv", sep='\t')

def main():

    args = commandline_args()
    sample_cluster_dict = create_sample_dictionary(args.clustered_scanpy_obj[0])
    generate_sample_umap_coords_output_file(sample_cluster_dict, args.output_sample_umap_coords[0])
